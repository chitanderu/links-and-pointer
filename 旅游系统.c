...
struct travel * search(struct travel*head, int number); //查找旅游记录 
struct travel * del(struct travel*head, int number);    //删除旅游记录 
void printone(struct travel*head, int number); //打印单个旅游记录
...
//定义函数 注意 上述函数为返回指针的函数，所以都带有*号


  struct travel  *addrecord(struct travel*head,struct travel*p) //一开始head为null p则指向动态分配内存的节点
{ 
      static  struct travel *q; 
      if(head==NULL)	{head=p; q=p;p->next=NULL;}
	   else   {q->next=p;q=p; p->next=NULL;}
...
void  printone(struct travel *head,int number)
{	
	struct travel  *p;
        p=head;
		while(p!=NULL){if(p->num==number) 
		
		 {
		 printf("  编号       姓名      出发日期     点赞数  \n");
			printf("   %-10d  %-10s   %d.%d        %d\n",p->num,p->name,p->month,p->day,p->support);}
		;p=p->next;}
	 //从头结点开始查找，找到后返回p，否者返回NULL
...
{	
	...
	for(i=0;i<18;i++)printf(" ");printf("◇◆◇     ");printf("8:查找单个旅游记录");printf("             ◇◆◇");printf("\n");
...
int main()
{
	...
	int b,k,choice,number, i,day,month,ch,j,count=0,count1=0,count2=0,flag1,flag2,flag3,flag4,flag5; 
	int size=sizeof(struct travel);//分配动态内存时计算需要的长度
	start_output();
  do{ 
	menue(); 
	printf("请选择:");
	...
	switch(choice){
	   ...
	   case 1:system("cls");
        printf("\n");count2=0;
		count=0;p=head;while(p!=NULL){p=p->next;count2++;}  //从头结点开始遍历，检测旅游记录添加的个数
		...
		p=head;
		while(p!=NULL){if(p->num==number)flag2=1;p=p->next;} //检测是否有重复的编号出现 。
		...
		if(flag3==1)printf("\t\t※日期错误，请重新输入※\n");}while(flag3==1);
		 ...
		 if(flag4==1) printf("\t\t※点赞数字异常请重新输入※\n");} while(flag4==1);
		  ...
		  p=(struct travel*)malloc(size);		  //分配动态内存 ，并用临时指针p指向这个结点
		  p->num=number;
		  strcpy(p->name,name);
		  p->month=month;
		  p->day=day;
